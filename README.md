...
<p align="center">
  <img src="https://img.shields.io/badge/版本-3.2.0-blue.svg" alt="版本">
  <img src="https://img.shields.io/badge/模块数量-571个-brightgreen.svg" alt="模块数量">
...
> **重要声明：本软件仅作为技术工具提供，用户必须遵守法律法规，开发者对用户的使用行为不承担任何责任。详见[免责声明](#免责声明)。**
...
WebRPA 是一款完全开源、可离线运行的可视化机器人流程自动化（RPA）工具。它以拖拽式工作流编辑器为核心，让用户无需编写代码，仅靠拖拽与连线即可搭建网页、桌面与手机端的自动化流程，覆盖数据采集、表单填写、文件处理、办公自动化与系统集成等场景。

项目内置 571 个功能模块与一位具备自愈能力的 AI 小助手：你可以用一句话描述需求，由 AI 自动搭建、排错并重跑工作流。所有运行环境（Python 3.13、Node.js、浏览器引擎、媒体与文档处理工具）均已随包内置，解压双击启动器即可使用，无需联网、无需额外安装，天然适配内网与局域网部署。

- **面向人群**：希望解放重复劳动的运营、测试、数据、办公与开发人员，无论是否具备编程基础。
- **核心定位**：零代码、可视化、开箱即用、完全离线的一站式自动化平台。
- **授权模式**：个人与非商业用途完全免费（AGPL-3.0），商业场景可获取商业授权。
...
[IPWO](https://www.ipwo.net/?ref=GitHubWebRPA) 住宅代理为 WebRPA 自动化任务提供多地区网络访问支持，适用于网页自动化、数据采集、网站访问及网络环境测试等场景。

覆盖 195+ 国家地区的住宅 IP 资源，用户可根据业务需求选择相应地区的 IP，为不同自动化任务提供更灵活的网络环境配置。**免费测试入口，90折扣码：0204**
...
- **模块丰富**：571 个功能模块，覆盖 95% 自动化场景（含基于 DrissionPage 的反检测网页自动化模块）
  > 模块数量口径：571 = 模块类型表（`moduleTypeLabels`）全部条目，减去 `custom_module`（自定义模块占位）与 `subflow_header`（子流程头，由执行器内部处理）两个伪类型；包含「分组」「便签」两个画布工具。文档、AI 提示词与自动化审计（`backend/tests/contract/test_module_inventory.py` 的 `DISCLOSED_MODULE_COUNT`）统一使用这一口径，新增或删除模块时审计会报出「实际数量 vs 文档数量」。
...
- **永不白屏**：全局错误边界兜底，任何模块异常都弹出错误卡片（含报错详情与 AI 诊断），绝不让编辑器整页白屏
- **可视化调试**：支持断点、单步执行，以及**从任意节点开始运行**（鼠标悬停节点点击 ▶ 即可从该节点起跑、跳过其上游，调试无需每次从头执行）
- **版本历史**：Git 式本地版本快照（含节点、连线、全局变量），可随时恢复 / 对比；小助手会在大改动前自动存档
- **WebDAV 远程存储**：工作流可保存/读取到 NAS、Nextcloud、坚果云等，多端共享
- **自定义快捷键**：常用功能（运行/停止/保存/新建等）可自由绑定组合键
...
- **一键打包 EXE**：任意工作流可一键编译为独立 Windows 可执行程序，内置可视化「EXE 运行界面设计器」自由摆放控件，双击即跑、无需对方安装环境
- **主题切换**：内置默认 / 暗色 / 灰色多套主题，界面随心切换
- **完全离线**：所有资源本地化，无需外网连接，完美支持局域网部署

### 关键模块能力

依托 571 个内置模块（按场景分门别类，拖入画布即可使用），WebRPA 覆盖以下关键能力：

- **网页自动化**：页面导航、元素交互与查询、数据采集、网络抓包，以及基于 DrissionPage 的反检测自动化
- **桌面自动化**：真实鼠标键盘模拟、图像识别点击、屏幕录制、桌面应用控件操作、系统操作
- **手机自动化**：Android 点击 / 滑动 / 输入、投屏镜像、应用管理、文件推拉、图像识别
- **流程控制与触发器**：条件、循环、子流程复用、断言、定时任务，Webhook / 热键 / 文件监控 / 邮件 / 图像 / 声音等多种触发器
- **数据处理**：变量运算、文本处理、列表 / 字典 / 数学统计、表格与 CSV
- **Excel 与数据库**：openpyxl 全套 Excel 自动化、WPS / 飞书多维表格，MySQL / PostgreSQL / Redis 等数据库
- **AI 能力**：AI 对话与视觉、AI 数据处理（抽取 / 分类 / 摘要 / 翻译 / 情感 / 规整 / 语义去重 / 智能路由）、AI 生成（文生图 / 文生视频）、AI 智能爬虫与元素定位、OCR 与验证码识别
- **媒体处理**：图像编辑、盲水印、视频与音频处理、媒体格式转换
- **文件与文档**：文件管理、文件 / 文件夹对比、PDF 处理（合并 / 拆分 / 加解密 / 水印 / 转 Word 等）、文档格式转换
- **集成与通信**：HTTP 请求、邮件、17 种消息推送通道、QQ / 微信机器人、SSH 远程、SAP 自动化、局域网共享
- **实用工具与测试**：加密编码、颜色与时间转换、脚本执行（JS / Python）、Allure 测试报告
...
- 后端服务：http://localhost:5241 （默认端口，可在启动器中配置）
- 前端服务：http://localhost:5921 （默认端口，可在启动器中配置）
...
从 2.5.0 起，WebRPA 支持**模块化分发**：重量级能力（OCR、语音识别、人脸手势、
媒体处理、文档转换、手机自动化、QQ 机器人等 13 个功能包，合计约 2.8GB）从核心包中
拆出，做成独立的 zip 功能包，按需安装：

1. 下载体积小得多的「核心包」解压启动；
2. 编辑器 → 右上角「更多」→ **功能模块包**，查看全部功能包与安装状态；
3. 需要什么能力就下载对应 zip，在管理界面「从本地路径安装」或「上传 zip 安装」，
   随装随用（无需重启）；不用了一键卸载释放磁盘。

- 从完整版升级的用户所有能力默认可用（探测真实文件，不依赖安装记录）；
- 未安装某包时，对应模块运行会明确提示「请安装 XX 功能模块包」，不影响其它功能；
- 详细设计与发布流程见 `docs/模块化分发体系说明.md`。
...
├── WebRPA备用启动.bat      # 备用启动脚本（无 WebView 环境下拉起前后端）
└── WebRPA停止服务.bat      # 配套停止脚本（按端口结束前后端进程）
```
...
- **AI数据处理**：信息抽取、文本分类、摘要、翻译、情感分析、数据规整、语义去重、智能路由（让工作流具备"判断力"）
- **AI视觉/爬虫**：图像理解、AI智能爬虫、AI元素定位
- **AI生成**：文生图、文生视频
...
**再次声明：任何用户使用 WebRPA 违反法律法规或侵犯他人权益的行为，均与 WebRPA 开发者无关，开发者不承担任何连带责任。**
...
<h4 align="center">支持一下WebRPA的独立开发工作</h4>
...
|  69  |         寒武纪         | 2026-06-20 14:41:33 |   5.00   |
|  70  |         包文正         | 2026-06-25 19:24:46 |  200.00  |
|  71  |          *氻           | 2026-06-26 08:18:21 |   9.90   |
|  72  |        神来之笔        | 2026-06-27 10:17:18 |  10.00   |
|  73  |       Peter.Yang       | 2026-06-27 20:59:42 |  20.00   |
|  74  |         等天黑         | 2026-06-30 10:30:43 |  666.66  |
|  75  |          *千           | 2026-07-01 10:07:50 |   8.80   |
|  76  |        勇往直前        | 2026-07-05 18:49:41 |  10.00   |
|  77  |          *叶           | 2026-07-10 13:59:49 |   2.00   |
|  78  |          *巴           | 2026-07-10 14:04:21 |   1.00   |
|  79  |        .何喜乐         | 2026-07-10 15:45:10 |  50.00   |
|  80  |          *人           | 2026-07-11 01:36:55 |   6.66   |
|  81  |          J*a           | 2026-07-11 09:19:06 |  10.00   |
|  82  |        ☆王小鉴☆        | 2026-07-11 09:53:09 |  50.00   |
|  83  |        ☆王小鉴☆        | 2026-07-11 10:35:32 | 1000.00  |
|  84  |        无懈可击        | 2026-07-11 19:09:32 |  300.00  |
|  85  |        无懈可击        | 2026-07-13 04:04:22 |  500.00  |
|  86  |        无懈可击        | 2026-07-13 21:34:05 |  200.00  |
|  87  |          *月           | 2026-07-13 15:29:05 |   6.66   |
|  88  |          段*C          | 2026-07-13 15:35:40 |  10.00   |
|  89  |          *的           | 2026-07-15 11:58:46 |   1.00   |
|  90  |        0度沸水         | 2026-07-17 13:09:46 |   6.00   |
|  91  |          *竞           | 2026-07-17 16:43:49 |  66.66   |
|  92  |          **宇          | 2026-07-18 20:38:39 |   1.00   |
|  93  |          **宇          | 2026-07-20 15:01:01 |   1.00   |
|  94  |          **懿          | 2026-07-26 19:02:04 |   0.10   |
|  95  |         王博飞         | 2026-07-26 19:09:02 |   5.00   |
|  96  |        氢锂迢迢        | 2026-07-26 20:05:49 |  10.00   |
|  97  |           jy           | 2026-07-26 20:38:12 |  100.00  |
|  98  |        IRON MAN        | 2026-07-26 20:56:01 |  18.88   |
|  99  |        无懈可击        | 2026-07-26 22:10:06 |  100.00  |
...
| 永久授权  | 适用场景               | 定价（元） |
|:----- |:------------------ |:-----:|
| 个人    | 自由职业者 / 个人工作室永久授权  | 1000  |
| 微型企业  | 10 人以下企业，多场景永久授权   | 1800  |
| 小型企业  | 10-30 人企业，多场景永久授权  | 3400  |
| 小型企业  | 30-50 人企业，多场景永久授权  | 4500  |
| 中大型企业 | 50-100 人企业，多部门永久授权 | 6200  |
| 中大型企业 | 100 人以上企业，规模化永久授权  | 10000 |
