...
- **Rich modules**: 571 functional modules covering 95% of automation scenarios (including DrissionPage-based anti-detection web automation modules)
  > How the count is defined: 571 = every entry in the module type table (`moduleTypeLabels`) minus two pseudo types — `custom_module` (a placeholder for user-defined modules) and `subflow_header` (handled internally by the executor). The two canvas tools, Group and Note, are included. Documentation, AI prompts and the automated audit (`DISCLOSED_MODULE_COUNT` in `backend/tests/contract/test_module_inventory.py`) all use this same definition, so adding or removing a module makes the audit report "actual count vs documented count".
...
Backed by 571 built-in modules (organized by scenario, drag onto the canvas to use), WebRPA covers the following key capabilities:
...
│   │   ├── executors/      # Module executors (core logic of 571 modules)
...
- **Detailed tutorials** covering all 571 modules
