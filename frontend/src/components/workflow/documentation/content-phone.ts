...
1. 在 X / Y 坐标输入框右侧点击 **「拾取坐标」** 按钮
2. 系统会自动启动手机镜像窗口（独立窗口、自动置顶到前台）
3. 在镜像窗口中**按住 Ctrl + 点击**目标位置即可自动填入 X、Y
4. 不按 Ctrl 时镜像窗口照常响应正常的手机操作，不会触发拾取
5. 拾取完成后会自动停止 picker，可以安全关闭镜像窗口

> 跨电脑兼容：拾取按钮内部使用基于设备真实分辨率的换算，无论显示器 DPI、镜像窗口大小、Windows 缩放比例如何变化，拿到的都是手机内坐标系的真实像素，不会再出现"在自己电脑能用、换台电脑就偏了"的问题。
