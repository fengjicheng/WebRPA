...
"Read cell" and "Read range" both have a **Read content** option:

| Option | What you get |
|------|----------|
| Value (default) | The cell's calculated result, e.g. \`=SUM(A1:A2)\` reads as \`3\` |
| Formula | The formula written in the cell, e.g. reads as \`=SUM(A1:A2)\` |

Value mode has one prerequisite: formula results are cached into the xlsx by Excel when it saves. If the file has never been opened and saved in Excel (for example it was generated by a program or by the "Create Excel" module), formula cells have no cached value. In that case the module **falls back automatically**, returns the formula text, and reports how many cells fell back in the run result. To get real calculated results, open the file in Excel and save it once.

### Dates and array formulas

Read results are normalised into plain text and numbers so they can be referenced directly with \`{variable}\`:

- Date cells read as \`2026-08-05 22:27:05\`; a date at midnight reads as \`2026-08-05\`
- Array formulas (multi-cell \`{=...}\` formulas) read as their formula text
- Rich-text cells read as plain text

If you need to compare or offset dates, use the date-time modules to parse the text back into a date first.
