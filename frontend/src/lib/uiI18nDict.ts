...
Object.assign(UI_DICT, {
  ...
  '扩展会直接加载到上方「浏览器」里选择的浏览器（Edge/Chrome），并保留你的登录态。': 'Extensions load directly into the browser selected in "Browser" above (Edge/Chrome), keeping your sign-in state.',
  '自动复制选择器到剪贴板': 'Auto-copy selector to clipboard',
  '开启后，在自动化浏览器里选择元素（单选或相似元素）时，自动把提取到的选择器复制到剪贴板': 'When enabled, picking an element in the automation browser (single or similar) automatically copies the extracted selector to the clipboard',
  '选择器已复制到剪贴板': 'Selector copied to clipboard',
  '选择器复制到剪贴板失败': 'Failed to copy selector to clipboard',
  '图像资源库为空（可选）': 'Image library is empty (optional)',
  '可直接在上方输入框填写图片路径，或点右侧按钮从电脑选择；也可到「图像」面板上传常用图片，方便下次快速选取。': 'Enter an image path in the box above, or click the button on the right to pick one from your computer; you can also upload frequently used images in the "Images" panel for quick reuse.',
  '可到「图像」面板上传常用图片，方便下次快速选取。': 'You can upload frequently used images in the "Images" panel for quick reuse.',
  '已检测到内置 Chromium，可作为扩展加载异常时的备用浏览器。': 'Bundled Chromium detected; it can serve as a fallback browser if extension loading fails.',
  '可选：如个别浏览器版本无法加载扩展，可在项目目录运行「Python313\\python.exe -m playwright install chromium」启用内置 Chromium 作为备用。': 'Optional: if a specific browser version fails to load extensions, run "Python313\\python.exe -m playwright install chromium" in the project directory to enable the bundled Chromium as a fallback.',
...
Object.assign(UI_DICT, {
  ...
  '从此节点开始运行（跳过其上游节点）': 'Run from this node (skip its upstream nodes)',
...
Object.assign(PHRASES, {
  ...
  "从指定节点开始执行工作流：": "Run workflow from the selected node: ",
  "工作流已从指定节点开始执行：": "Workflow started from the selected node: ",
  " - 副本 ": " - copy ",
...
Object.assign(UI_DICT, {
  ...
  '夸克网盘高速下载': 'Fast download (Quark Drive)',
  '各功能包右侧「下载」可直达夸克网盘高速下载': 'The "Download" button on each pack jumps straight to a high-speed Quark Drive download',
  ...
  '· 功能包是覆盖到 WebRPA 安装目录的增量 zip：安装即解压、卸载即删除。大多数功能包安装后即可直接使用；少数含大型库的包（OCR / 图像 / 语音 / 数据表格）若未立即生效，到 WebRPA 启动器先点「停止服务」再点「启动服务」重启一次即可。': '· A feature pack is an incremental zip overlaid onto the WebRPA install directory: installing extracts it and uninstalling deletes it. Most packs work right after installing; for a few packs with large libraries (OCR / image / speech / data tables), if they do not take effect immediately, restart once in the WebRPA launcher by clicking "Stop services" and then "Start services".',
  '安装完成。大多数功能包可直接使用，无需重启；但含大型库的功能包（如 OCR / 图像 / 语音 / 数据表格）若运行时仍提示缺包，请到 WebRPA 启动器中先点「停止服务」、再点「启动服务」重启一次后再试。': 'Installed. Most packs work without a restart; but for packs with large libraries (e.g. OCR / image / speech / data tables), if a module still reports a missing pack at runtime, restart once in the WebRPA launcher by clicking "Stop services" then "Start services", and try again.',
  ...
  '打开 WebRPA 的 GitHub Releases 页面下载功能模块包': 'Open the WebRPA GitHub Releases page to download feature packs',
  '打开夸克网盘高速下载此功能包': 'Open Quark Drive to download this feature pack at high speed',
  '打开夸克网盘下载功能包': 'Open Quark Drive to download feature packs',
  // 缺少功能包弹窗
  '缺少功能模块包': 'Missing feature packs',
  '当前工作流用到的部分模块需要先安装对应功能包才能运行': 'Some modules used by this workflow require their feature packs to be installed before it can run',
  '受影响模块：': 'Affected modules: ',
  '以下功能包任装其一即可满足：': 'Installing any one of the following packs is enough:',
  '如何安装功能模块包（新手指引）': 'How to install feature packs (beginner guide)',
  '1. 点击上方对应功能包的「下载」按钮，在打开的夸克网盘页面把 .zip 功能包下载到本地（无需解压）。': '1. Click the "Download" button of the pack above and, on the Quark Drive page, download the .zip pack to your computer (no need to unzip).',
  '2. 点击本弹窗底部「打开功能包管理器」。': '2. Click "Open pack manager" at the bottom of this dialog.',
  '3. 在管理器里选择安装方式：小包可直接「上传 zip 安装」；大包推荐「从本地路径安装」，把下载好的 zip 完整路径粘进去点安装。': '3. Pick an install method in the manager: for small packs use "Install from zip upload"; for large packs use "Install from local path" and paste the full path of the downloaded zip, then click Install.',
  '4. 安装完成后回到编辑器重新点击运行；若仍提示缺包，到 WebRPA 启动器先点「停止服务」再点「启动服务」重启一次即可生效。': '4. After installing, go back to the editor and click Run again; if it still reports a missing pack, restart once in the WebRPA launcher by clicking "Stop services" then "Start services".',
  '提示：功能包只需安装一次，之后同类工作流都可直接运行；从完整版升级的用户通常已自带全部功能包。': 'Tip: each pack only needs installing once; afterwards similar workflows run directly. Users upgrading from the full edition usually already have every pack.',
  '稍后再说': 'Later',
  '打开功能包管理器': 'Open pack manager',
  '工作流缺少功能模块包，已弹窗提示下载与安装方式，运行已中止': 'The workflow is missing feature packs; a dialog with download and install instructions has been shown, and the run was aborted',
})
...
Object.assign(UI_DICT, {
  ...
  '监控页加载工作流失败：「${workflowId}」内容格式无法解析。': 'Monitor page failed to load the workflow: the content of "${workflowId}" could not be parsed.',
  "计划任务本次执行：${ok ? '成功' : '失败'}": "Scheduled task run: ${ok ? 'succeeded' : 'failed'}",
  '计划任务正在执行中，执行完成后会自动补齐完整日志…': 'The scheduled task is still running; the full log will be filled in automatically once it finishes…',
  '已同步后端执行产生的 ${synced} 个变量到「全局变量」面板': 'Synced ${synced} variables produced by the backend run into the Global Variables panel',
